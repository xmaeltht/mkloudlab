...
output "minio_oidc_client_secret" {
  description = "The client secret for MinIO OIDC client"
  value       = keycloak_openid_client.oidc_clients["minio"].client_secret
  sensitive   = true
}
...
# SAML client outputs
output "jenkins_saml_client_id" {
  description = "The client ID for Jenkins SAML client"
  value       = keycloak_saml_client.jenkins.client_id
}
...
output "client_redirect_uris" {
  ...
  value = merge(
    ...
    {
      "jenkins" = keycloak_saml_client.jenkins.valid_redirect_uris
